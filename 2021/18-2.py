...
with open(filename) as f:
    ...
    fish = list(map(json.loads, lines))
...
perms = list(permutations(fish, 2))
sf_list = list(map(lambda combi: redu(SnailFish(None, combi[0], 0, False), SnailFish(None, combi[1], 0, False)), perms))
results = list(map(lambda f: f.calc_magnitude(), sf_list))
